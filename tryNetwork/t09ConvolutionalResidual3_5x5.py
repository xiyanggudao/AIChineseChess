...
def addLayer(x, in_channels, out_channels, kSize=5):
	global layer_cnt
	layer_cnt += 1
	w1 = tf.Variable(tf.random_uniform([kSize, kSize, in_channels, out_channels], -0.01, 0.01))
	...
	if in_channels == out_channels:
		layer_cnt += 1
		w2 = tf.Variable(tf.random_uniform([5, 5, in_channels, out_channels], -0.01, 0.01))
