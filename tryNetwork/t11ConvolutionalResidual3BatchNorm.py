...
def addLayer(x, in_channels, out_channels, kSize=5):
	...
	norm1 = tf.layers.batch_normalization(z1)
	...
	tf.summary.histogram("weight_bias/znorm" + str(layer_cnt), norm1)
	y = tf.nn.relu(norm1)
	if in_channels == out_channels:
		...
		norm2 = tf.layers.batch_normalization(z2)
		...
		tf.summary.histogram("weight_bias/znorm" + str(layer_cnt), norm2)
		y = tf.nn.relu(norm2)
	return y
...
def pickySoftmax(x, inputSize, outputSize, pickySwitch):
	...
	z = tf.matmul(x, w) + b
	norm = tf.layers.batch_normalization(z)
	...
	tf.summary.histogram("weight_bias/znorm" + str(layer_cnt), norm)
